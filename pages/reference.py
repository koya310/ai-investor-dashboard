...
from components.shared import L, P, W, card_title

fresh = _dm.get_data_latest_dates()

st.title("システム仕様")
st.caption("判定基準・計算式・実行ルール・障害時の見方")


with st.container(border=True):
    card_title("運用スコープ", color=P)
    ...
    st.caption(
        f"画面更新時刻: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )


with st.container(border=True):
    card_title("Go/No-Go 判定仕様", color="#d97706")
    st.markdown(
        f"""
| KPI | 目標値 | 計算式 | 判定の意味 |
|---|---:|---|---|
...
with st.container(border=True):
    card_title("パイプライン仕様", color=P)
    st.markdown(
        """
| Step | 入力 | 出力 | 確認ポイント |
|---|---|---|---|
| 1. 情報収集 | ニュースAPI/RSS | `news` | 件数が0でないか |
| 2. AI分析 | ニュース・銘柄群 | `ai_analysis` | 方向性が偏りすぎていないか |
| 3. 売買判断 | テクニカル+AI分析 | `signals` | 確信度が低すぎないか |
| 4. 注文執行 | シグナル+リスク制約 | `trades` | 実行漏れがないか |
